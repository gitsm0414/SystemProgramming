...
<uname(2)>
#include<sys/utsname.h> //Unix Timesharing System(uts)
...
struct sysinfo {
    long uptime;             // 부팅 후 경과된 시간 (단위: 초)
    unsigned long loads[3];  // 최근 1분, 5분, 15분간의 평균 시스템 부하율 (Load Average)
    unsigned long totalram;  // 설치된 총 물리 메모리(RAM) 크기
    unsigned long freeram;   // 현재 사용 가능한(비어 있는) 물리 메모리 크기
    unsigned long sharedram; // 공유 메모리로 사용 중인 메모리 크기
    unsigned long bufferram; // 커널 버퍼(임시 저장소)로 사용 중인 메모리 크기
    unsigned long totalswap; // 총 스왑(Swap) 파티션/파일의 크기
    unsigned long freeswap;  // 현재 남아 있는 스왑 공간의 크기
    unsigned short procs;    // 현재 시스템에서 실행 중인 총 프로세스의 개수
    unsigned long totalhigh; // 총 하이 메모리(High Memory) 크기 (32비트 시스템용)
    unsigned long freehigh;  // 사용 가능한 하이 메모리 크기 (32비트 시스템용)
    unsigned int mem_unit;   // 메모리 크기 계산을 위한 단위 (바이트 수, 예: 1024)
    char _f[20-2*sizeof(long)-sizeof(int)]; // 향후 확장을 위한 패딩 (64비트 호환용)
};
...
#include<unistd.h>
    ...
    int seteuid(uid_t uid); //어떠한 uid를 EUID로 설정해준다. 일반사용자는 real UID 혹은 saved UID 설정만 가능
...
기준 시간으로부터 경과한 시간을 micro-second(10^-6)단위로 읽고/씀.
tz는 일반적으로 NULL사용, 자세한 내용은 man 참조
성공 0, 실패 -1
...
extern int daylight; //DST(일광시?)적용 여부, 역사적으로 한번이라도 적용한 적 있는가?
...
struct tm {
   ...
   int tm_isdst;       /* 일광 절약 시간제(DST) 현재 적용 여부 */
};
